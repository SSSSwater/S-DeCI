...
项目应当将源代码、实验入口、OpenSpec 元数据、脚本和文档放置在清晰分离的顶层目录或文件中。 ? Requirement SHALL ??????
...
系统应当使用 `run_cv.py` 作为配置和运行交叉验证实验的主入口。 ? Requirement SHALL ??????
...
实验层应当负责模型构建、设备选择、数据获取、训练、验证、checkpoint 管理和指标汇报。 ? Requirement SHALL ??????
...
`data_provider/data_loader_CV.py` 中的数据集类应当根据不同数据集的目录结构加载预处理后的 fMRI 样本和标签。 ? Requirement SHALL ??????
...
`data_provider/data_factory_CV.py` 应当创建分层 k 折训练和验证 dataloader。 ? Requirement SHALL ??????
...
项目应当将 `models/DeCI.py` 和 `layers/DeCI_Layer.py` 视为实际设计的模型实现。 ? Requirement SHALL ??????
...
除 `models/DeCI.py` 以外的模型文件，以及除 DeCI 专用层实现以外的层文件，应当被视为性能对照基线，除非它们被 DeCI 直接引用。 ? Requirement SHALL ??????
...
`scripts/` 目录应当按照模型家族或对照类别组织复现实验命令。 ? Requirement SHALL ??????
...
实验脚本应当将运行输出重定向到 `logs/<dataset>/<model>/` 风格的目录中。 ? Requirement SHALL ??????
...
`extract_re.py` 应当从 `logs/` 树中汇总最佳日志指标。 ? Requirement SHALL ??????
...
项目应当将 OpenSpec 工作流配置保存在 `openspec/` 下。 ? Requirement SHALL ??????
...
项目级 Markdown 文档应当放在 `docs/` 下。 ? Requirement SHALL ??????
