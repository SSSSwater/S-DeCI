...
DeCI SHALL 面向多变量 fMRI 时间序列分类任务。模型先将每个 ROI/channel 的时间维序列独立嵌入到潜在维度，然后堆叠一个或多个 DeCI block。每个 block 从当前残差表征中逐步提取趋势分量和季节性分量；每个分量分别通过独立分类头产生 logits；最终预测由所有趋势分量和季节性分量的 logits 相加得到。
...
DeCI 模型 SHALL 由 `run_cv.py` 传入的实验参数对象进行配置。
...
模型 SHALL 在二分类任务中使用一维输出，在多分类任务中使用类别数维输出。
...
DeCI 模型 SHALL 接收形状为 `[B, T, N]` 的 fMRI 样本张量。
...
模型 SHALL 在嵌入前支持可选地沿时间维对每个样本进行归一化。
...
模型 SHALL 将每个 ROI/channel 的时间序列独立嵌入到 `d_model` 维。
...
模型 SHALL 将 `configs.layer` 个 DeCI block 依次作用于残差表征。
...
模型 SHALL 通过相加所有趋势 logits 与所有季节性 logits 来融合分量预测。
...
`DeCI_Block` SHALL 处理形状为 `[B, N, D]` 的嵌入残差张量。
...
每个 DeCI block SHALL 使用 `Trend_ext` 从当前残差表征中提取趋势分量。
...
每个 DeCI block SHALL 使用 `Seasonal_ext` 从去除趋势后的残差中提取季节性分量。
...
每个 DeCI block SHALL 分别对趋势分量和季节性分量进行分类。
...
`Trend_ext` SHALL 使用沿潜在维度的一维逐通道卷积估计平滑趋势分量。
...
`Trend_ext` SHALL 保持输入张量的 `[B, N, D]` 形状。
...
`Seasonal_ext` SHALL 使用可学习 gate 与残差 MLP 来估计残差表征中的季节性分量。
...
`Seasonal_ext` SHALL 使用残差前馈块进一步细化门控表征。
...
DeCI 模型 SHALL 兼容 `Exp_Main` 中的二分类训练与验证路径。
...
DeCI 模型 SHALL 兼容 `Exp_Main` 中的多分类训练与验证路径。
...
DeCI 实现 SHALL 只依赖 PyTorch 与 DeCI layer 模块。
...
对照模型 SHALL 仍可通过 `Exp_Basic.model_dict` 使用，但 MUST NOT 被视为 DeCI 子模块。
