...
> 本文同时保留实验设计与最终取舍。第 2、3 节所述互补视图是已经完成验证的历史实验方案；当前规范性结论以“最终实验决策”为准，正式模型只执行标准视图、可靠 TP EMA 和可选多阶因果编码。
...
这不是 Sinkhorn 的强制均衡分配。某 prototype 没有可靠样本时保持不动；诊断会报告长期未更新 prototype，而不是人为把样本分给它。分配到 `(k,m)` 的可靠点以切空间均值形成目标：

$$
\bar z^{\tan}_{k,m}=
\frac{1}{|\mathcal R_{k,m}|}
\sum_{b\in\mathcal R_{k,m}}\log_0^c(z_b).
$$
...
默认配置：`hpec_ema_start_epoch=20`、`hpec_reliable_confidence_threshold=0.70`、`hpec_ema_alpha=0.995`、`hpec_ema_anchor_weight=0.10`、`hpec_reliable_min_samples=2`。prototype 参数在该模式下 `requires_grad=False`，避免 autograd 和 EMA 同时移动同一参数。

选择原因：该机制保留 BrainCL Eq. (13) 中“TP 样本 + EMA”的可靠原型更新思想，但根据完整消融结果移除无正收益的双视图一致性门槛，同时保留多 prototype 和 HPEC 流形几何。替代方案是保持 Sinkhorn；其优势是覆盖均衡，缺点是在不可靠样本多时会强迫错误样本参与更新。采用 energy winner 而非再次加入均衡约束，满足“用 TP 样本单独训练移动且替换 Sinkhorn”的要求。

### 2. 历史实验：因果显著性互补视图仅位于模块 2 之后
...
历史实验曾缓存标准和互补输出；当前 `S_DeCI` 只缓存标准输出并保持 `forward()` 主返回 logits 的兼容性。训练循环只在 `model.training`、batch 为训练 batch、完成 `optimizer.step()` 后调用 `update_reliable_prototypes_after_step()`；验证、测试、可视化前向均严格冻结 prototype。

当前 TensorBoard 只保留 `PrototypeUpdate/*` 与 `CausalReachability/*`：记录可靠 TP 比例、每类/每 prototype 更新数、prototype 位移、未更新 prototype 数、多 hop gate 与编码范数。最后 epoch 只保存标准训练/测试 t-SNE 和 prototype 分配统计；真实标签仅用于训练更新和可视化标注。

已完成的实验顺序为：当前基线、可靠 TP EMA、可靠 TP EMA + 互补视图（无一致性损失）、再启用一致性损失、最后单独叠加多阶编码。最终只保留可靠 TP EMA 和可选多阶编码；完整结论写入 `result.xlsx` 与证据台账。
...
完整 MDD/AAL116 5-fold、50 epoch 消融完成后，候选机制按证据分为三类：

1. **正式保留**：`reliable_tp_ema`。prototype 只使用标准视图中预测正确且置信度达标的训练样本，在 `optimizer.step()` 后以无梯度 EMA 更新；它不依赖总 loss，也不调用 Sinkhorn。
2. **默认关闭的独立消融**：多阶因果可达性编码。该路径仍保持 $A_{\mathrm{cls}}[parent,child]$ 方向，不改写图，只在模块 3 输入端加入小 hop 残差。
3. **完整负向后移除**：因果显著性互补遮挡、标准/互补第二次模块 3/4 前向、Poincare 一致性、InfoNCE 和 masked CE。最佳组合未超过可靠 TP EMA 基线，且 InfoNCE 接近 $\log(B)$，表明没有学到有效实例对应。
...
可靠 prototype 更新是上述前向之后的独立状态更新，不是第二条前向分支：
...
\Pi_{\mathbb{B}_c}
\left(
\alpha p_{k,m}+(1-\alpha)\bar{z}_{k,m}^{\mathrm{reliable}}
\right),
\qquad
\text{no grad}.
$$

历史互补公式仅用于解释已完成实验，不再代表当前可调用能力。
